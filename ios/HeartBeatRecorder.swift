...
@objc(HeartBeatRecorder)
class HeartBeatRecorder : NSObject,AVCaptureVideoDataOutputSampleBufferDelegate{
  ...
  var state:RecordState = .STARTED;
  
  var waitingValues:[Bool] = [];
  ...
  func testEvent(){
    //print("toto");
    self.bridge.eventDispatcher().sendAppEvent( withName: "end_recording", body: 60 )
    //EventSender().notifyReact();
  }
  ...
  func startRecording(){
    ...
    self.bridge.eventDispatcher().sendAppEvent( withName: "start_initializing", body: colors )
    ...
    self.state = .WAITING;
    self.bridge.eventDispatcher().sendAppEvent( withName: "start_waiting", body: colors )
  ...
  func stopRecording(){
    ...
    self.bridge.eventDispatcher().sendAppEvent( withName: "end_recording", body: colors )
    self.state = .ENDED;
    //self.bridge.eventDispatcher().sendAppEvent( withName: "end_recording", body: colors )
  }
  ...
  func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputSampleBuffer sampleBuffer: CMSampleBuffer!, from connection: AVCaptureConnection!)
  {
    ...
    switch self.state {
    case .WAITING:
      let newValues:Float = self.getWaitingValue(img: img);
      let valid = self.isFingerPlaced();

      
      if (valid){
        self.state = .RECORDING;
        self.bridge.eventDispatcher().sendAppEvent( withName: "start_recording", body: colors )
      }
      
      //print(newValues)
    case .RECORDING:
      let newValues = self.getRowvalues(img: img);
      nbFrame = nbFrame + 1;
      if isFingerPlaced(){
        if (nbFrame >= Int(self.fps*60))
        {
          print("END TIME")
          stopRecording();
  ...
  func getWaitingValue(img : CGImage) -> Float{
  ...
  func checkFinger(r: CGFloat,g:CGFloat,b:CGFloat ){
    var maxCount = 4;
    var valid:Bool = (r>150) && (g<100)&&(b<100);


    self.waitingValues.append(valid);

    if self.waitingValues.count>maxCount{
      self.waitingValues.removeFirst();
    }


  }

  func isFingerPlaced() -> Bool {
    var maxCount = 4;
    var res:Bool = true;
